...
export default function Pagination({ totalPages, currentPage, handleClick }: PaginationProps){
  return (
    ...
    <Button
      ...
      className={`${currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'} px-4 py-2 rounded-md hover:bg-blue-600 transition-colors duration-300 shadow-md`}
    >
      {index + 1}
    </Button>
  ))}
