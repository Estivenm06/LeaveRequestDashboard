...
export default function Pagination({ totalPages, currentPage, handleClick }: PaginationProps){
  return (
    ...
    <Button
      ...
      className={`${currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'} px-2 py-1 md:px-4 md:py-2 rounded-md hover:bg-blue-600 transition-colors duration-300 shadow-md text-center`}
    >
      {index + 1}
    </Button>
  ))}
